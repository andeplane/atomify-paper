...
The next step now is to create a larger cylinder, the one containing the red atoms in figure \ref{fig:cylinder_simulation}, where regular time integration will take place.
...
\end{lstlisting}
The expected result as can be seen in figure \ref{fig:initial_configuration}d.
...
All these groups are highlighted in figure \ref{fig:initial_configuration}e by hovering the box containing all the groups. in Atomify as shown in figure \ref{fig:initial_configuration}f.
The colors of the atoms matches the group color next to the group name in figure \ref{fig:initial_configuration}f, where the order of coloring is the same as they were defined in the script.
Therefore we see no green atoms (\code{cylinder1}) since all of these are in the groups \code{cylinder2}, \code{cylinder3}, \code{nve} and \code{gas}.
