...
These can be plotted as a function of time and visualized while the 
simulation is running.
Quantities that are measured per atom can be plotted in a histogram.
...
Performance improvements are planned by for instance using geometry shaders to
generate periodic copies.
Currently this is partially done on the CPU,
which  reduces performance drastically.
We also aim to include more backends, both using the CPU and GPU,
in addition to running simulations on computing clusters while visualizing them
